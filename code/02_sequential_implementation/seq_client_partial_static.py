...
class VanillaPartialClientStatic:
    ...
    def end_training(self):
        pd.DataFrame.from_dict(self.record).to_csv(
            f"resources/results/partial_static/{self.dataset}/client_{self.client_id}.csv",
...
if __name__ == "__main__":
    print(f"EXPERIMENT: PARTIAL STATIC")
    for dataset in DATASET:
        print(f"DATASET: {dataset.upper()}")
        indexes = fetch_index(dataset)
        (x_train, y_train), (x_test, y_test) = fetch_dataset(dataset)

        fl_server = Server(NODES, 'partial_static', dataset)
        clients = [VanillaPartialClientStatic(i, EPOCHS, dataset, indexes[i], x_train, y_train, x_test, y_test) for i in
                   range(NODES)]
