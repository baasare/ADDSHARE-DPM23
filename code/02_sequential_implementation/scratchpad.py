...
from helpers.utils import fetch_dataset, get_dataset_x, decode_layer
from helpers.constants import CHUNK_SIZE, NODES, ROUNDS, EPOCHS, DATASET
from helpers.utils import TimingCallback, get_public_key, get_private_key, NumpyDecoder
from helpers.utils import generate_additive_shares, perform_exchange, NumpyEncoder, fetch_index
...
class AdditiveClient:
    ...
    def end_training(self):
        pd.DataFrame.from_dict(self.record).to_csv(
            f"resources/results/additive_rsa/{self.dataset}/client_{self.client_id}.csv",
            index=False, header=True)
        self.model.save(f"resources/models/additive_rsa/{self.dataset}/client_{self.client_id}_lenet_5.h5")
