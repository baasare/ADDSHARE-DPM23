...
from helpers.constants import CHUNK_SIZE, NODES, ROUNDS, DATASET
from helpers.utils import generate_groups, get_dataset_x, fetch_index, fetch_dataset
from helpers.utils import generate_additive_shares, perform_exchange, NumpyEncoder
from helpers.utils import decode_layer, TimingCallback, get_public_key, get_private_key, NumpyDecoder
...
class AdditiveClient:
    ...
    def end_training(self, group):
        pd.DataFrame.from_dict(self.record).to_csv(
            f"resources/results/additive_groups_rsa_{group}/{self.dataset}/client_{self.client_id}.csv",
