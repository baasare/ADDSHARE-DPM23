...
class AdditiveClient(Node):
    ...
    def disconnect(self):
        pd.DataFrame.from_dict(self.record).to_csv(f"resources/results/additive/{self.dataset}/client_{self.client}.csv",
                                                   index=False,
                                                   header=True)
        # self.model.save(f"resources/models/additive/{self.dataset}/client_{self.client}_lenet_5.h5")
        self.stop()
...
if __name__ == "__main__":
    ...
    client = AdditiveClient(ADDRESS, NODE_PORT, client_id=int(sys.argv[1]) - 1, epochs=1, max_connections=20,
                            dataset=DATASET[1])
    client.start()
