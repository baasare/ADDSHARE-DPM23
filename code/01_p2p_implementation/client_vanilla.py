...
class VanillaClient(Node):
    ...
    def disconnect(self):
        pd.DataFrame.from_dict(self.record).to_csv(f"resources/results/vanilla/{self.dataset}/client_{self.client + 1}.csv",
                                                   index=False, header=True)
        # self.model.save(f"resources/models/vanilla/{self.dataset}/client_{self.client}_lenet_5.h5")
        self.stop()
