...
from code.helpers.constants import MESSAGE_START_TRAINING, MESSAGE_FL_UPDATE, MESSAGE_END_SESSION, ADDRESS, SERVER_PORT, DATASET
...
class VanillaClient(Node):
    ...
    def start_training(self, global_model):
        ...
        # print(f'Round ({self.round}) Client ({self.client}) Training Time: {self.current_training_time}')
    ...
    def disconnect(self):
        pd.DataFrame.from_dict(self.record).to_csv(f"resources/results/partial_2/{self.dataset}/client_{self.client + 1}.csv",
                                                   index=False, header=True)
        # self.model.save(f"resources/models/partial_2/{self.dataset}/client_{self.client}_lenet_5.h5")
        self.stop()
...
if __name__ == "__main__":
    ...
    client = VanillaClient(ADDRESS, NODE_PORT, client_id=int(sys.argv[1]) - 1, epochs=1, max_connections=6, dataset=DATASET[2])
    client.start()
