...
from code.helpers.utils import get_dataset, encode_layer, decode_layer, get_public_key, generate_groups
...
class Server(Node):
    ...
    def start_round(self):
        ...
        self.groupings = generate_groups(self.nodes_inbound.copy(), GROUPINGS)
    ...
    def end_session(self):
        ...
        pd.DataFrame.from_dict(self.record).to_csv(f"resources/results/{self.client_type}/{self.dataset}/0_server.csv",
                                                   index=False,
                                                   header=True)
        self.send_to_nodes(json.dumps(data))
        self.stop()
    ...
    def send_connected_nodes(self, node):
        ...
        result = set()
        for sublist in self.groupings:
            if node in sublist:
                result.update(sublist)
        result = list(result)
        self.grouping_time = temp_start_time - timer()

        for requested_node in result:
...
if __name__ == "__main__":
    NODES = 20
    ROUNDS = 10
    CLIENT_TYPE = ['vanilla', 'additive', 'additive_rsa', 'partial_1', 'partial_2']

    fl_server = Server(ADDRESS, SERVER_PORT, ROUNDS, SERVER_ID, NODES, CLIENT_TYPE[1], DATASET[2])
    fl_server.start()
