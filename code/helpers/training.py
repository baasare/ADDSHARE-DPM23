...
def get_lenet5(dataset):
    ...
    model.add(Conv2D(filters=6, kernel_size=(5, 5), strides=1, activation='relu', name=f'conv2d_{0}', input_shape=(pixel, pixel, rgb)))
    model.add(AveragePooling2D(pool_size=(2, 2), strides=(2, 2)))

    # Convolutional layer 2
    model.add(Conv2D(filters=16, kernel_size=(5, 5), activation='relu', name=f'conv2d_{1}'))
    model.add(AveragePooling2D(pool_size=(2, 2), strides=(2, 2)))
    ...
    model.add(Dense(units=120, activation='relu', name=f'dense_{0}'))

    # Fully connected layer 2
    model.add(Dense(units=84, activation='relu', name=f'dense_{1}'))

    # Output layer
    model.add(Dense(units=10, activation='softmax', name=f'dense_{2}'))
    return model
...
def get_lenet3(dataset):
    ...
    model = Sequential()
    # Convolutional layer 1
    model.add(Conv2D(filters=6, kernel_size=(5, 5), strides=1, activation='relu', input_shape=(pixel, pixel, rgb)))
    model.add(AveragePooling2D(pool_size=(2, 2), strides=(2, 2)))
