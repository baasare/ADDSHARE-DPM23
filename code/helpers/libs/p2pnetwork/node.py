...
from code.helpers.libs.p2pnetwork.nodeconnection import NodeConnection
...
class Node(threading.Thread):
    ...
    def __init__(self, host, port, id=None, callback=None, max_connections=1):
        ...
        try:
            ...
            # Handle any exceptions that occur during the thread execution
        ...
        self.clients = max_connections

        # Debugging on or off!
        self.debug = False

        # Add a flag to track if the thread is terminated
        self.is_terminated = False

    def stop(self):
        """Terminates the connection and stops the thread."""
        self.is_terminated = True

    def __del__(self):
        """Cleanup method called when the object is deleted."""
    ...
    def connect_get_node(self, host, port, reconnect=False):
        """ Make a connection with another node that is running on host with port. When the connection is made,
            an event is triggered outbound_node_connected. When the connection is made with the node, it exchanges
            the id's of the node. First we send our id and then we receive the id of the node we are connected to.
            When the connection is made the method outbound_node_connected is invoked. If reconnect is True, the
    ...
    def run(self):
        ...
        while not self.terminate_flag.is_set():  # Check whether the thread needs to be closed
            try:
                ...
                if len(self.nodes_inbound) < self.clients:
