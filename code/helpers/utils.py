...
from code.helpers.constants import FIELD_SIZE, BIT_SIZE, DATASET
...
def get_public_key(client_id):
    # print(f"PUBLIC ID: {client_id}")
    curren_wd = os.path.join(os.getcwd(), 'resources', 'keys', 'client_' + str(client_id) + '_public.pem')

    with open(curren_wd, 'rb') as f:
        return serialization.load_pem_public_key(f.read())


def get_private_key(client_id):
    # print(f"PRIVATE ID: {client_id}")
    curren_wd = os.path.join(os.getcwd(), 'resources', 'keys', 'client_' + str(client_id) + '_private.pem')
...
def get_dataset(client_id, dataset="cifar-10", iid="iid", balanced='balanced'):
    curren_wd = os.getcwd()
    path = curren_wd + f'/resources/dataset/{dataset}/{iid}_{balanced}.txt'
    ...
    if dataset == "cifar-10":
        ...
        train_data = sio.loadmat(curren_wd + f'/resources/dataset/svhn/train_32x32.mat')
        test_data = sio.loadmat(curren_wd + f'/resources/dataset/svhn/test_32x32.mat')
...
def fetch_dataset(dataset):
    if dataset == "cifar-10":
        ...
        train_data = sio.loadmat(current_wd + f'/resources/dataset/svhn/train_32x32.mat')
        test_data = sio.loadmat(current_wd + f'/resources/dataset/svhn/test_32x32.mat')
        x_train = np.array(train_data['X'])
        y_train = np.array(train_data['y'])
...
def fetch_index(dataset):
    current_wd = os.getcwd()
    path = current_wd + f'/resources/dataset/{dataset}/iid_balanced.txt'
...
def combine_csv_files(folder_path):
    # folder_path = "resources/results/vanilla/cifar-10"
...
def combine_find_mean(folder_path):
    folder_items = os.listdir(folder_path)

    for folder in folder_items:
        for dataset in DATASET:
            combine_csv_files(os.path.join(folder_path, folder, dataset))
